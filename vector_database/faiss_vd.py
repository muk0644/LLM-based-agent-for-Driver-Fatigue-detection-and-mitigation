...
class PrefixVectorDB:
    ...
    def add_vector(self, token_matrix: np.ndarray, intervention: str, source: str):
        ...
        vector = token_matrix.reshape(-1)  # [20480]
        ...
        if len(self.metadata) > 0:
            ...
            for idx, dist in zip(I[0], D[0]):
                ...
                neighbor_vec = neighbor.get("vector")
                if neighbor_vec is not None:
                    cosine_dist = self.cosine_distance(vector, neighbor_vec)
        ...
        self.metadata[self.current_id] = {
            ...
            'source': source,
            'vector': vector,  # ✅ store actual vector here
            'timestamp': time.time(),
    ...
    def search(self, query_token_matrix: np.ndarray, k=5):
        ...
        query_vector = self.normalize_vector(query_token_matrix.reshape(-1))  # [20480]
